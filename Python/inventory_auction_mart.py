...
logging.basicConfig(
    filename='inventory_auction_mart_log.log',
    filemode= 'a',
...
query = sql_queries.inventory_auction_mart_sql

df = pd.read_sql_query(query, engine_out)

df.to_sql(
    name = 'test_inventory_auction_mart',
    con = engine_in,
    index = False,
    if_exists = 'replace'
)
logging.info("There are {} inventories".format(len(df)))
